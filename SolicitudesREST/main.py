...
async def consultarOpcion(idOpcion:int,db:Session=Depends(get_db)):
    opcion=models.Opcion()
    return opcion.consultarPorId(idOpcion,db)
@app.post('/opciones',tags=["Opciones"],summary="Crear Opcion",response_model=Salida)
async def crearOpcion(opcionInsert:OpcionInsert,db:Session=Depends(get_db))->Any:
...
async def modificarOpcion(opcionU:Opcion,db:Session=Depends(get_db))->Any:
...
async def eliminarOpcion(idOpcion:int,db:Session=Depends(get_db))->Any:
    opcion=models.Opcion()
...
async def agregarSolicitud(solicitudI:SolicitudInsert,db:Session=Depends(get_db),
...
async def modificarSolicitud(s:SolicitudUpdate,db:Session=Depends(get_db),
                       salida:UsuarioSalida=Depends(autenticar))->Any:
    if salida.estatus == 'OK' and salida.usuario.tipo == 'Alumno':
        solicitud=models.Solicitud(idSolicitud=s.idSolicitud,idAlumno=s.idAlumno,
                               tema=s.tema,idOpcion=s.idOpcion)
        respuesta=solicitud.modificar(db)
        return respuesta
    else:
        if salida.estatus=='OK':
            salida.estatus='Error'
...
async def cancelarSolicitud(s:SolicitudCE,db:Session=Depends(get_db),
                      salida:UsuarioSalida=Depends(autenticar))->Any:
    if salida.estatus == 'OK' and salida.usuario.tipo == 'Alumno':
        solicitud=models.Solicitud(idSolicitud=s.idSolicitud,idAlumno=s.idAlumno)
...
async def enviarSolicitud(s:SolicitudCE,db:Session=Depends(get_db),
                    salida:UsuarioSalida=Depends(autenticar))->Any:
    if salida.estatus == 'OK' and salida.usuario.tipo == 'Alumno':
        solicitud=models.Solicitud(idSolicitud=s.idSolicitud,idAlumno=s.idAlumno)
...
async def revisarSolicitud(s:SolicitudRevisar,db:Session=Depends(get_db),
                     salida:UsuarioSalida=Depends(autenticar))->Any:
    if salida.estatus == 'OK' and salida.usuario.tipo == 'Administrativo':
        solicitud=models.Solicitud(idSolicitud=s.idSolicitud,
                                   ...
                                   estatus=s.estatus)
        respuesta=solicitud.revisar(db)
        return respuesta
    else:
        if salida.estatus=='OK':
            salida.estatus='Error'
...
async def consultarSolicitudes(db:Session=Depends(get_db),
                         salida:UsuarioSalida=Depends(autenticar))->Any:
...
async def consultarSolicitud(idSolicitud:int,db:Session=Depends(get_db),
                       salida:UsuarioSalida=Depends(autenticar))->Any:
    if salida.estatus == 'OK':
        solicitud=models.Solicitud()
...
async def consultarSolicitudesPorAlumno(idAlumno:int,db:Session=Depends(get_db),
                                  salida:UsuarioSalida=Depends(autenticar))->Any:
    if salida.estatus == 'OK' and salida.usuario.tipo == 'Alumno':
        solicitud=models.Solicitud()
...
async def consultarSolicitudesPorCoodinador(idCoordinador:int,db:Session=Depends(get_db),
                                      salida:UsuarioSalida=Depends(autenticar))->Any:
    if salida.estatus == 'OK' and salida.usuario.tipo == 'Administrativo':
        solicitud=models.Solicitud()
